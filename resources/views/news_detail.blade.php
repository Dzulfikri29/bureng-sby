                        ...
                        <div class="col-md-6 col-sm-6 col-lg-3">
                            <aside class="sidebar w-100">
                                <div class="widget2 w-100">
                                    <h3 class="widget-title2">BERITA LAIN</h3>
                                    <div class="mini-posts-wrap w-100">
                                        @foreach ($other_news as $other_news_data)
                                            <div class="mini-post-box d-flex flex-wrap w-100">
                                                <a href="{{ route('news-detail', ['slug' => $other_news_data->slug]) }}" title=""><img class="img-fluid w-100 blog-thumb" src="{{ asset('storage/' . $other_news_data->gallery->path) }}" alt="{{ $other_news_data->gallery->name }}"></a>
                                                <div class="mini-post-info">
                                                    <span class="d-block thm-clr">{{ Carbon\Carbon::parse($news->created_at)->format('d M Y') }}</span>
                                                    <h4 class="mb-0"><a href="{{ route('news-detail', ['slug' => $other_news_data->slug]) }}" title="">{{ $other_news_data->name }}</a></h4>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
