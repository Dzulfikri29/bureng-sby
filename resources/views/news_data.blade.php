            ...
            <div class="col-md-6 col-sm-6 col-lg-4">
                <div class="post-box w-100">
                    <div class="post-img overflow-hidden position-relative w-100">
                        <a href="{{ route('news-detail', ['slug' => $news_data->slug]) }}" title=""><img class="img-fluid w-100 gallery-size" src="{{ asset('storage/' . $news_data->gallery->path) }}" alt="{{ $news_data->gallery->name }}"></a>
                    </div>
                    <div class="post-info position-relative w-100">
                        <a class="thm-bg" href="{{ route('news-detail', ['slug' => $news_data->slug]) }}" title=""><i class="fas fa-link"></i></a>
                        <span class="post-date thm-clr">{{ Carbon\Carbon::parse($news_data->date)->format('d F Y') }}</span>
                        <h3 class="mb-0"><a href="{{ route('news-detail', ['slug' => $news_data->slug]) }}" title="">{{ $news_data->name }}</a></h3>
