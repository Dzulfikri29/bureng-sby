new TomSelect('#blog_category_id', {
    ...
    load: function (query, callback) {
        var url = `${base_url}/blog-category/select`;
        fetch(url, {
                method: 'POST',
                headers: {
...
Dropzone.options.blogImageUpload = {
    paramName: 'image',
    textTarget: null,
    maxFileSize: 1024,
...
const get_blog_images = () => {
    $('#fancyboxGallery').html('');
    $.ajax({
        url: `${base_url}/admin/blog-image`,
        method: "GET",
        headers: {
            'X-CSRF-TOKEN': token,
        },
                                ...
                                <div class="card card-sm">
                                    ...
                                    <div class="card-body">
                                        <div class="row col-divider text-center">
                                            ...
                                            <div class="col">
                                                <a class="text-danger" href="javascript:;" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Delete" data-bs-original-title="Delete" onclick="delete_blog_image('${e.image}')">
                                                    <i class="bi-trash"></i>
                                                </a>
                                            </div>
                                        </div>
...
const delete_blog_image = (name) => {
    $.ajax({
        url: `${base_url}/admin/blog-image-delete`,
        method: "POST",
        headers: {
            'X-CSRF-TOKEN': token,
        },
