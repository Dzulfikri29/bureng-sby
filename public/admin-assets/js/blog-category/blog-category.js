$(document).on('ready', function () {
    ...
    // =======================================================
    HSCore.components.HSDatatables.init($('#datatable-blog-category'), {
        ajax: {
...
$("#datatable-blog-category").css("width", "100%");
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.success) {
                ...
                $('#form-data')[0].reset();
                $('#datatable-blog-category').DataTable().ajax.reload();
            }
            name_input.val('');
        },
        error: function (res) {
...
const edit_data = (id) => {
    $('.validation-error-message').text('').addClass('d-none');
    formModalTitle.text('edit blog category');
    $.ajax({
        url: `${base_url}/admin/blog-category/${id}/edit`,
        method: "GET",
        headers: {
            'X-CSRF-TOKEN': token,
        },
        success: function (res) {
            ...
            $('#form-data').attr('action', `${base_url}/admin/blog-category/${res.id}`);
...
const add_data = () => {
    $('.validation-error-message').text('').addClass('d-none');
    $('#form-data').attr('action', `${base_url}/admin/blog-category`);
    formModalTitle.text('tambah blog category');
    formModal.modal('show');
    method.val('POST');
}
