$(document).on('ready', function () {
    ...
    // =======================================================
    HSCore.components.HSDatatables.init($('#datatable-product-category'), {
        ajax: {
...
$("#datatable-product-category").css("width", "100%");
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.success) {
                ...
                $('#form-data')[0].reset();
                $('#datatable-product-category').DataTable().ajax.reload();
            }
            name_input.val('');
        },
        error: function (res) {
...
const edit_data = (id) => {
    $('.validation-error-message').text('').addClass('d-none');
    formModalTitle.text('edit product category');
    $.ajax({
        ...
        success: function (res) {
            ...
            $('#form-data').attr('action', `${base_url}/admin/product-category/${res.id}`);
...
const add_data = () => {
    $('.validation-error-message').text('').addClass('d-none');
    $('#form-data').attr('action', `${base_url}/admin/product-category`);
    formModalTitle.text('tambah product category');
    formModal.modal('show');
    method.val('POST');
}
